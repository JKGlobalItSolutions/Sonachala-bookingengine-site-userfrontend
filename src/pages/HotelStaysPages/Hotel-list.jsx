...
const HotelListing = () => {
  ...
  const fetchHotelsAndPreloadPrices = async () => {
    ...
    try {
      ...
      // Get current booking parameters
      ...
      const hotelsData = await Promise.all(
        hotelsSnapshot.docs.map(async (docSnapshot) => {
          const hotelData = { id: docSnapshot.id, ...docSnapshot.data(), type: "Hotel" }
          const roomDetails = await fetchRoomDetails("Hotels", docSnapshot.id)
          ...
          // Store the total price and pricing breakdown
          return { 
            ...hotelData, 
            price: pricing.total,
    ...
    try {
      ...
      if (roomSnapshot.docs.length > 0) {
        ...
        return {
          ...
          discount: Number.parseFloat(roomData.discountValue || roomData.discount) || 0,
          maxGuestAllowed: Number.parseInt(roomData.maxGuestAllowed || roomData.maxGuests) || 2,
        }
      }
      return { roomPrice: 0, perAdultPrice: 0, perChildPrice: 0, discount: 0, maxGuestAllowed: 2 }
    } catch (error) {
